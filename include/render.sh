...
# @summary Print the rc.d glue of the STARTUP and SHUTDOWN hooks: the rc script /usr/local/etc/rc.d/elvbootd (PROVIDE elvbootd, REQUIRE NETWORKING syslogd motd -- replay and summary need the log, the notice must follow motd(8), KEYWORD shutdown) whose start runs hook.startup.sh and whose stop runs hook.shutdown.sh -- each ':' when its phase binds nothing
# @group   foundation
# @internal
# @see     stage elvbootd mk
# @see     stage elvbootd glue devd
...
_stage_elvbootd_glue_rcd1() {
        ...
        printf '# PROVIDE: elvbootd\n# REQUIRE: NETWORKING syslogd motd\n# KEYWORD: nojail shutdown\n\n. /etc/rc.subr\n\n'

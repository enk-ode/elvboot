...
# Options (before the positional args): --maxprocs N runs the tests in
# parallel (xargs -P N, each job its own suite process and sandbox). The
# per-function phases (syntax per anchor, the hulls, KEEP_GOING, read-only)
# are expanded into one job PER ANCHOR, so the wall time is the longest
# single anchor, not the longest phase. Default: every CPU (a
# serial run of 2000 tests took 13 hours; nobody chooses that on purpose);
# --maxprocs 1 is the classic sequential run.
...
#-----------------------------------------------------------------------------
# Command Walking Functions (the former scripts/elebake-walkthrough.sh; the walkthrough of a command is elebake-walkthrough.sh, the binary's third name)
#-----------------------------------------------------------------------------
...
EOF
  ...
  # batch 1 goes on past the failed line (inherited: the checks of an
...
# READONLY_SKIP_FUNCTIONS - functions the read-only test does not run.
#
# Empty: every function is run under a cat pin on a
# writable database with a marker touched before, and a file changed
# outside .tmp and .log is a failure -- a generation-time writer. The 39
# names that stood here "legitimately writing at generation time" were
# run with the list emptied: none writes. The one that did,
# environment cache on, is an act now (render > .new, place). A name
# put back here needs the reason beside it.
READONLY_SKIP_FUNCTIONS=""
...
EOF
  ...
  local marker="$TEST_BASE_DIR/readonly-marker-$$"
  touch "$marker"
  if [ -z "$expanded_args" ]; then
  ...
  # generation-time writers: the function ran under a cat pin, so nothing
  # may have changed in the database except the execution artifacts
  local written
  written=$(find "$TEST_DIR" \( -path "$TEST_DIR/.tmp" -o -path "$TEST_DIR/.log" \) -prune -o -newer "$marker" -print 2>/dev/null | sed "s|^$TEST_DIR/||" | grep -v '^$' | tr '\n' ' ')
  rm -f "$marker"
  if [ -n "$written" ]; then
    fail "${prefix}${func}: writes at generation time (under a cat pin): $written"
    return 1
  fi
...
#
# test_readonly_one <func> <terminal|combinator|batch> -- one function's
# read-only check as a job of its own (the parallel runner expands
# test_readonly_database into these)
test_readonly_one() {
  test_header "Read-Only Database: $2 $1"
  test_setup
  create_messy_realistic_database
  test_readonly_function "$1" "$2"
}
  ...
  for f in $(_help_source_files); do
    ...
    awk -v OFS='	' '
      function emit(type, fn) {
        ...
        else if (tagblob ~ /@defcompletion/) kind="defcompletion"
...
test_help_blocks_conform() {
  ...
  local recs problems="" allowed=" command summary group param option returns example see since defgroup order parent topic internal env completion defcompletion "
  recs=$(_help_records)
...
test_help_graph_resolves() {
  ...
  # Completion sources: every <placeholder> in a @command usage resolves to a
  # source -- through @completion in its block or the @defcompletion table --
  # and every source is one complete_values() (include/help.sh) knows. A
  # <a|b> token whose parts are all unlisted is a literal alternative and
  # passes; a single <name> must be listed.
  local vocab=" files dirs stage medium key backend foundation-claim foundation-gate foundation-policy foundation-trigger foundation-expectation foundation-macro database env-var interpreter-fn profile layer command-path none "
  local defc pair nm src usage over tok parts part hit
  defc=$(printf '%s\n' "$tags" | sed -n 's/^# *@defcompletion  *\([^ ]*\)  *\([a-z-]*\).*/\1 \2/p')
  for pair in $(printf '%s\n' "$defc" | tr ' ' '='); do
    nm=${pair%%=*}; src=${pair#*=}
    case "$vocab" in *" $src "*) ;; *) problems="$problems
  @defcompletion '$nm' uses unknown source '$src'" ;; esac
  done
  local tmpc; tmpc=$(mktemp 2>/dev/null || echo "/tmp/help_compl.$$")
  printf '%s\n' "$recs" | awk -F'	' '$1=="B" && $4=="command"' > "$tmpc" || true
  local type fn sentinel kind tagblob
  while IFS='	' read -r type fn sentinel kind tagblob; do
    [ -n "${type:-}" ] || continue
    usage=$(printf '%s' "$tagblob" | tr ';' '\n' | sed -n 's/^# *@command  *//p' | head -1)
    over=$(printf '%s' "$tagblob" | tr ';' '\n' | sed -n 's/^# *@completion  *\([^ ]*\)  *\([a-z-]*\).*/\1 \2/p')
    for pair in $(printf '%s\n' "$over" | tr ' ' '='); do
      nm=${pair%%=*}; src=${pair#*=}
      case "$vocab" in *" $src "*) ;; *) problems="$problems
  @completion '$nm' in '$fn' uses unknown source '$src'" ;; esac
    done
    for tok in $(printf '%s\n' "$usage" | tr ' ' '\n' | grep '<' | tr -d '[]'); do
      hit=0; parts=$(printf '%s' "$tok" | tr '|' ' ' | tr -d '<>')
      for part in $parts; do
        printf '%s\n' "$over" | cut -d' ' -f1 | grep -qx "$part" && hit=1
        printf '%s\n' "$defc" | cut -d' ' -f1 | grep -qx "$part" && hit=1
      done
      [ "$hit" -eq 1 ] && continue
      case "$tok" in *"|"*) continue ;; esac   # literal alternatives
      problems="$problems
  placeholder $tok in '$usage' has no @completion/@defcompletion source"
    done
  done < "$tmpc"
  rm -f "$tmpc"
  if [ -z "$problems" ]; then
    pass "Help graph resolves (groups, see-refs, uniqueness, completion sources)"
  else
...
# symptom: (1) a `case` on an argument whose branches
...
parallel_main() {
  ...
  # One job per anchor for the parameterised phases: a job token is
  # <test function>:<argument>[:<argument>], run by main() directly.
  local jobs="" f
  for t in $tests; do
    case "$t" in
      test_all_combinator_syntax)
        for f in $COMBINATOR_FUNCTIONS; do jobs="$jobs test_combinator_function:${f#__}"; done ;;
      test_all_batch_combinator_syntax)
        for f in $BATCH_COMBINATOR_FUNCTIONS; do jobs="$jobs test_batch_combinator_function:${f#___}"; done ;;
      test_all_terminal_output)
        for f in $TERMINAL_FUNCTIONS; do jobs="$jobs test_terminal_function:${f#_}"; done ;;
      test_all_batch_error_propagation_hull)
        for f in $BATCH_COMBINATOR_FUNCTIONS; do jobs="$jobs test_batch_combinator_transitive_hull:${f#___}"; done ;;
      test_all_batch_keep_going_precise)
        for f in $BATCH_COMBINATOR_FUNCTIONS; do jobs="$jobs test_batch_combinator_keep_going_precise:${f#___}"; done ;;
      test_readonly_database)
        for f in $TERMINAL_FUNCTIONS; do is_readonly_skip_function "${f#_}" || jobs="$jobs test_readonly_one:${f#_}:terminal"; done
        for f in $COMBINATOR_FUNCTIONS; do is_readonly_skip_function "${f#__}" || jobs="$jobs test_readonly_one:${f#__}:combinator"; done
        for f in $BATCH_COMBINATOR_FUNCTIONS; do is_readonly_skip_function "${f#___}" || jobs="$jobs test_readonly_one:${f#___}:batch"; done ;;
      *) jobs="$jobs $t" ;;
    esac
  done
  tests=$jobs
  echo "$(printf '%s\n' $tests | wc -l | tr -d ' ') jobs"
  # The job token travels as an argument ($4), not through -I: xargs(1)
  # refuses a replaced command line beyond 255 bytes (13.09., after 219 jobs).
...
# The refactored modules follow the top-down pattern: every
...
main() {
  # A job token of the parallel runner (<test>:<argument>[:<argument>]) runs
  # in THIS process and is never dispatched again: the worker is serial.
  case "$TEST_FILTER" in *:*) MAXPROCS=1 ;; esac
  ...
  # A job token of the parallel runner: run the named test with its
  # argument(s), then the summary -- no phase loop, no filter.
  case "$TEST_FILTER" in
    *:*)
      if [ ! -f "$TEST_SCRIPT" ]; then
        echo "${COLOR_RED}ERROR:${COLOR_RESET} $TEST_SCRIPT not found"
      ...
      local job fn a1 a2
      for job in $TEST_FILTER; do
        fn=${job%%:*}; a1=${job#*:}; a2=""
        case "$a1" in *:*) a2=${a1#*:}; a1=${a1%%:*} ;; esac
        if [ -n "$a2" ]; then "$fn" "$a1" "$a2"; else "$fn" "$a1"; fi
      done
      test_summary
      return $?
      ;;
